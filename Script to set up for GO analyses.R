...
go.meanings <- suppressMessages(AnnotationDbi::select(GO.db, unique((tbl(my_db, "bee_go") %>% 
                                                                       dplyr::select(GO) %>% 
                                                                       collect(n=Inf) %>% 
...
entrez.tbl <- entrez.tbl %>% filter(!(is.na(beebase1) & is.na(beebase2))) %>% filter(!is.na(entrez.id))
entrez.tbl <- tbl(my_db, "rsem_am") %>% select(gene) %>% collect(n=Inf) %>% left_join(entrez.tbl, by = c("gene" = "beebase2")) %>% select(gene, entrez.id) 
...
kegg.tbl <- data.frame(entrez.id = kegg.tbl$value,
                       ...
                       meaning = substr(kegg.tbl$L1, 10, nchar(kegg.tbl$L1)), stringsAsFactors = FALSE)

# Make a table of all the kegg terms we know for the genes in our set
kegg.tbl <- entrez.tbl %>% left_join(kegg.tbl) %>% filter(!is.na(kegg)) %>% select(-entrez.id) %>% mutate(kegg = gsub("ame", "", kegg))
...
library(KEGG.db)
keggframeData <- tbl(my_db, "bee_kegg") %>% dplyr::select(kegg, gene) %>% collect(n=Inf) %>% as.data.frame()
