...
# Experiment: expression relative to the max observed for that species
# n.per.species <- as.numeric(table(treatments$species))
# tpm <- tpm * apply(tpm, 2, function(x) 1 / unlist(mapply(rep, tapply(x, treatments$species, max), each = n.per.species)))
