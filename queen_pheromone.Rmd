...
#### This plot illustrates that 4 of the _Lasius_ samples are highly different to all of the rest
...
#### Make a table and plot of the correlations for each pair of species
```{r}
...
for(i in 1:nrow(species.combinations)){
  ...
  fc.data[,3:4] <- fc.data[,3:4] %>% log10 # Log the fold changes
...
species.combinations <- data.frame(sp1 = species.combinations[,1],
                                   sp2 = species.combinations[,2],
                                   rho = rho,
                                   p = p,
...
# Make a table
species.combinations %>% mutate(rho = format(round(rho, 2), nsmall = 2)) %>% pander

# Make a figure
species.combinations %>% ggplot(aes(sp1, sp2, fill=rho)) + geom_tile(colour="white", size=4) + scale_fill_gradient2(name = "Corr", low = brewer.pal(9, "RdYlBu")[7], mid = "white", high = brewer.pal(9, "RdYlBu")[2]) + 
...
### There is some overlap of pheromone-sensitive alternatively-spliced genes
```{r}
venn(list(Apis = am.alt.splice$name,`L. flavus` = lf.alt.splice$name,`L. niger` = ln.alt.splice$name))
```
...
inspect.module.genes <- function(module){
  ...
  bt <- tbl(my_db, "ebseq_gene_bt") %>% rename(bt = gene) %>% left_join(tbl(my_db, "bt2am"))
  gene.names$bt_fc <- bt$PostFC[match(gene.names$gene, bt$am)]
  lf <- tbl(my_db, "ebseq_gene_lf") %>% rename(lf = gene) %>% left_join(tbl(my_db, "lf2am"))
  gene.names$lf_fc <- lf$PostFC[match(gene.names$gene, lf$am)]
  ln <- tbl(my_db, "ebseq_gene_ln") %>% rename(ln = gene) %>% left_join(tbl(my_db, "ln2am"))
...
There are only two differentially expressed genes for B. terrestris, so it is only meaningful to test for enriched GO terms in the other three species.
...
### Enriched GO terms for differentially expressed genes in _L. niger
```{r}
...
### Venn diagram showing number of over-lapping differentially expressed genes
```{r echo = FALSE}
...
### GO term enrichment for genes that are affected by queen pheromone in 2 or 3 species
```{r}
...
```{r echo=FALSE}
inspect.module.genes(4) %>% pander(split.cell = 40, split.table = Inf)
```

```{r echo=FALSE}
unlink("blockwiseTOM-block.1.RData") # Delete this to save memory
```
