...
```{r fig.width=2.9, fig.height=2.54}
...
species.combinations$sig[species.combinations$p < 0.001] <- "**"
...
treatments.network.plot <- function(dat){ 
  dat %>% ggplot(aes(Species, Eigengene, fill = Treatment)) + 
    ...
    xlab(NULL) + 
    scale_x_discrete(labels = c("Apis\nmellifera", "Bombus\nterrestris", "Lasius\nflavus", "Lasius\nniger")) + theme_bw() + 
    theme(legend.position=c(0.9,0.1), panel.grid = element_blank(),strip.background = element_blank(), axis.text = element_text(face = "italic"))
}
