...
author: "Queen pheromones have conserved effects on gene expression in social insects separated by 150MY of evolution"
output:
  html_document:
...
mod4 <- inspect.module.genes(4) 
mod4 %>% pander(split.cell = 40, split.table = Inf)
```
...
melt(mod4[,c(2,4:7)]) %>% 
  ...
  scale_x_discrete(labels = c("Apis\nmellifera", "Bombus\nterrestris", "Lasius\nflavus"), expand=c(0,0)) +
  scale_y_discrete(expand=c(0,0)) + theme_bw() + theme(panel.grid = element_blank())
