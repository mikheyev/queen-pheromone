...
**Table S`r numbering("supp.table.counter")`**: List of significantly differentially expressed genes (EBseq; p < 10^-5^) in _Bombus terrestris_, listed in order of fold change in gene expression on a Log$_2$ scale. Positive fold change values indicate higher expression in the control, while negative values indicate higher expression in the queen pheromone treatment.
 <br/><br/> 
<input type=button class=hideshow></input>
```{r}
...
morandin.orthology <- read.csv("data/Morandin to Holman orthology.csv", stringsAsFactors = FALSE)
