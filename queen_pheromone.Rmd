...
if(!("most.pheromone.sensitive.genes.rds" %in% list.files())){
  ...
  # most.pheromone.sensitive.genes[[2]] <- most.pheromone.sensitive.genes[[2]] %>%
  #   mutate(`Exp. overlaps` = round(`Exp. overlaps`, 2), 
  #          `O/E` = round(`O/E`, 2))
}
```
...
for(i in 1:nrow(species.combinations)){
  ...
  plot_data <- fc.data
  names(plot_data)[3:4] <-c("s1", "s2")
  axis_names <- species.combinations[i, ]
  axis_names[axis_names == "am"] <- "Apis mellifera"
  axis_names[axis_names == "bt"] <- "Bombus terrestris"
  axis_names[axis_names == "lf"] <- "Lasius flavus"
  axis_names[axis_names == "ln"] <- "Lasius niger"
  log_fold_plots[[i]] <- ggplot(plot_data, aes(s1, s2)) +
    geom_point(alpha=0.3) + 
    stat_smooth(method = "lm", colour = "tomato") + 
    xlab(axis_names[1]) + ylab(axis_names[2])
...
### Table of correlations between species
...
### Plot of correlations between species
```{r FigureS1, fig.width=11}
# Make the heat map figure
...
grid.arrange(do.call("grid.arrange", log_fold_plots), heat.map.figure, ncol = 2, widths = c(1.5,1))
ggsave(grid.arrange(do.call("grid.arrange", log_fold_plots), heat.map.figure, ncol = 2, widths = c(1.5,1)), file = "figures/Correlations heat map.pdf", height = 4, width=4.3)
```
 <br/><br/> 
 **Figure S`r numbering("supp.figure.counter")`**: Scatterplots and the associated matrix of Spearman's rank correlations examining the correlation in pheromone sensitivity for each pair of species. The heat map colour shows the strength of the correlation (Spearman's rho, given on the heatmap) while the asterisks show the statistical significance after adjustment to control the false discovery rate (\*\*\* p < 1e^-6^).
